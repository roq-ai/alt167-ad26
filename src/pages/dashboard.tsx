...
import { getItTechnicians } from 'apiSdk/it-technicians';
...
import { ItTechnicianListPage as ShowCaseList } from 'pages/it-technicians';

function DashboardPage() {
  const { session } = useSession();

  const itTechnicianStat = useSWR<PaginatedInterface<ItTechnicianInterface>>(
    () => `/it-technicians`,
    () => getItTechnicians({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: itTechnicianStat?.data?.totalCount,
      isLoading: itTechnicianStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest It Technicians
                </Text>
                <NextLink href={`it-technicians`} passHref legacyBehavior>
                  <Button
